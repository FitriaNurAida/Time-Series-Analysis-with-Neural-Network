...
training=as.ts(y4t[1:144])     #membagi menjadi data training
testing=as.ts(y4t[145:168])    #membagi menjadi data testing
...
sd.Yt   = sd(training)
Yt_std <- scale(y4t)
Yt_std1 <- as.data.frame(scale(y4t))
library(DataCombine)
data1=slide(Yt_std1, Var = "V1", slideBy = -c(1,2,23))
colnames(data1)=c("yt","yt1","yt2","yt23")
data=data1[24:144,]
...
best.model_NN=list
fits.model_NN=matrix(0,nrow(data),length(neuron))
...
{
  set.seed(1)
  best.model_NN=neuralnet(formula=form,data=data,hidden=neuron[k],
...
write.csv(fore.model_NN, file="forecast4 NN.csv")
...
{
  akurasi[i,1]=accuracy(fits.model_NN[,i],training[24:144])[1,2]
  akurasi[i,2]=accuracy(fits.model_NN[,i],training[24:144])[1,3]
  akurasi[i,3]=accuracy(fits.model_NN[,i],training[24:144])[1,5]
...
write.csv(akurasi, file="akurasi 4 NN.csv")
...
a=min(min(fits.model_NN),min(training[24:144]))   #batas bawah plot data training
b=max(max(fits.model_NN),max(training[24:144]))   #batas atas plot data training
...
#PLOT DATA TRAINING#
plot(as.ts(training[24:144]),ylab="Yt",xlab="t",lwd=2,axes=F,ylim=c(a*1.1,b*1.1))
box()
...
#PLOT DATA TRAINING#
#Neuron 1#
plot(as.ts(training[24:144]),ylab="Yt",xlab="t",lwd=2,axes=F,ylim=c(a*1.1,b*1.5))
box()
...
axis(side=2,lwd=0.5,cex.axis=0.8,las=2)
axis(side=1,lwd=0.5,cex.axis=0.8,las=0,at=c(1,seq(35,400,35)))
lines(as.ts(fits.model_NN[,10]),col="yellow2",lwd=2)
legend("topleft",c("Data aktual","Data ramalan"),
...
axis(side=2,lwd=0.5,cex.axis=0.8,las=2)
axis(side=1,lwd=0.5,cex.axis=0.8,las=0,at=c(1:24),labels=c(145:168))
lines(as.ts(fore.model_NN[,10]),col="yellow2",lwd=2)
legend("topleft",c("Data aktual","Data ramalan"),
       col=c("black","yellow2"),lwd=2,cex=0.7)
